...
- **UI & Console Verification**: For any web application or frontend component created, execute `node D:\code\antigravity-cli\browser_runner\inspect.js <URL>` in headless Chromium.
...
- **Graphify Radar First**: For codebases larger than 10 files, query `D:\code\antigravity-cli\graphify_runner.py <path>` or read `graphify-out/graph.json` first to prevent context hallucination.
...
- **GitHub Actions Heavy Offloading**: Offload heavy compilation (C++, Rust) and extensive test suites to GitHub Actions workflows in `Ddos-spec` repositories (`D:\code\antigravity-cli\github_workflows\heavy_offload.yml`).
